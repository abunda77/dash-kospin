        ...
        <div class="grid md:grid-cols-2 gap-3">
                ...
                <div class="bg-white rounded-lg shadow p-4">
                    ...
                    <div class="space-y-2">
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-800">{{ $tabunganData['nasabah']['nama_lengkap'] }}</p>
                        </div>
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <p class="text-xs text-gray-600">No. Telepon</p>
                                <p class="text-sm font-semibold text-gray-800">{{ $tabunganData['nasabah']['phone'] ?? '-' }}</p>
                            </div>
                            @if($tabunganData['nasabah']['email'])
                            <div>
                                <p class="text-xs text-gray-600">Email</p>
                                <p class="text-sm font-semibold text-gray-800 truncate">{{ $tabunganData['nasabah']['email'] }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <p class="text-sm font-semibold text-gray-800">{{ $tabunganData['rekening']['no_tabungan'] }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-600">Status</p>
                                <span class="inline-block px-2 py-0.5 text-xs font-semibold rounded {{ $tabunganData['rekening']['status'] === 'aktif' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <p class="text-sm font-semibold text-gray-800">{{ $tabunganData['rekening']['produk'] }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-600">Tanggal Buka</p>
                                <p class="text-sm font-semibold text-gray-800">{{ $tabunganData['rekening']['tanggal_buka'] }}</p>
                            </div>
                        </div>
                        @if($tabunganData['rekening']['usia_rekening'])
                        <div>
                            <p class="text-xs text-gray-600">Usia Rekening</p>
                            <p class="text-sm font-semibold text-blue-600">{{ $tabunganData['rekening']['usia_rekening']['formatted'] }}</p>
                            <span class="inline-block mt-1 px-2 py-0.5 text-xs font-medium rounded {{ $tabunganData['rekening']['usia_rekening']['is_in_contract'] ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800' }}">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-bold text-green-600">{{ $tabunganData['rekening']['saldo_formatted'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-3">
                    ...
                    <div class="space-y-2">
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <p class="text-sm font-semibold text-gray-800">{{ $tabunganData['transaksi_terakhir']['kode_transaksi'] }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-600">Jenis</p>
                                <span class="inline-block px-2 py-0.5 text-xs font-semibold rounded {{ $tabunganData['transaksi_terakhir']['jenis_transaksi'] === 'setoran' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        ...
                        <div>
                            ...
                            <p class="text-lg font-bold text-gray-800">{{ $tabunganData['transaksi_terakhir']['jumlah_formatted'] }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-600">Tanggal</p>
                            <p class="text-sm font-semibold text-gray-800">{{ $tabunganData['transaksi_terakhir']['tanggal_transaksi'] }}</p>
                            @if($tabunganData['transaksi_terakhir']['mbg_eligibility'])
                            <div class="mt-2">
                                <span class="inline-block px-2 py-1 text-xs font-semibold rounded {{ $tabunganData['transaksi_terakhir']['mbg_eligibility']['eligible'] ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $tabunganData['transaksi_terakhir']['mbg_eligibility']['status'] }}
                                </span>
                                <p class="text-xs text-gray-500 mt-1">{{ format_hari_lalu($tabunganData['transaksi_terakhir']['mbg_eligibility']['days_ago']) }} yang lalu</p>
                            </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="text-center py-3">
                    ...
                    <p class="text-sm text-gray-600">Nomor tabungan ini sudah melakukan checkout untuk hari ini.</p>
                </div>
                @else
                <button 
                    wire:click="checkout"
