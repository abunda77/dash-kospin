...
<body>
    ...
    <div class="stats-grid">
        <div class="stat-card">
            <div class="stat-value">{{ number_format($stats['total_accounts'] ?? 0) }}</div>
            <div class="stat-label">Total Rekening Aktif</div>
        </div>
        <div class="stat-card">
            <div class="stat-value">{{ \App\Helpers\PdfHelper::formatCurrency($stats['total_balance'] ?? 0) }}</div>
            <div class="stat-label">Total Saldo</div>
        </div>
        <div class="stat-card">
            <div class="stat-value">{{ \App\Helpers\PdfHelper::formatCurrency($stats['avg_balance'] ?? 0) }}</div>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach($savingsData as $index => $tabungan)
                @php
        ...
        <tfoot>
            <tr>
                ...
                <th class="col-saldo currency" style="font-weight: bold;">{{ \App\Helpers\PdfHelper::formatCurrency($totalSaldo) }}</th>
                <th colspan="2"></th>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="footer">
        Total Rekening: {{ is_countable($savingsData) ? count($savingsData) : (method_exists($savingsData,'count') ? $savingsData->count() : 0) }} | Total Saldo: {{ \App\Helpers\PdfHelper::formatCurrency($totalSaldo) }}
    </div>
</body>
</html>